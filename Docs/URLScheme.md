...
For example by chaining it with a semicolon after a command when `terminal=false` can't be used ([full example](https://github.com/matryer/bitbar-plugins/blob/master/System/downloads.1h.sh)). Hint: Passing `--background` into open
will prevent bitbar from stealing focus when refreshing.
